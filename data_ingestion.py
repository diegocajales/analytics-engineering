...
def download_file(year, month):
    url = f"{BASE_URL}_{year}-{month}.csv.gz"
    file_path = os.path.join(DOWNLOAD_DIR, f"{BASE_FILE}_{year}-{month}.csv.gz")
...
if __name__ == "__main__":
    ...
    pairs = list(product(YEARS, MONTHS))
    with ThreadPoolExecutor(max_workers=4) as ex:
        file_paths = list(ex.map(lambda ym: download_file(*ym), pairs))
